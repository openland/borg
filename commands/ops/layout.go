...
func loadEdgedCenters(poly geometry.Polygon2D, smallSide float64, largeSide float64, rotate bool) Layout {
	for _, e := range poly.EdgesVectors() {
		...
		if left != nil && right != nil {
			...
			startTop := middleTop.Shift(geometry.Point2D{X: id.DX * (-largeSide / 2), Y: id.DY * (-largeSide / 2)})
			endTop := middleTop.Shift(geometry.Point2D{X: id.DX * (largeSide / 2), Y: id.DY * (largeSide / 2)})
